...
\end{equation}
\begin{equation}
C(G)=\int_x p_{data}(\textbf{x}) \left[log\frac{p_{data}(\textbf{x})}{\left(p_{data}(\textbf{x})+p_g(\textbf{x})\right)/2}\right]-\int_x\left[p_{data}(\textbf{x})log2\right]dx+\int_x p_g(\textbf{x})\left[log\frac{p_{g}(\textbf{x})}{\left(p_{data}(\textbf{x})+p_g(\textbf{x})\right)/2}\right]-\left[\int_x p_g(\textbf{x})log 2\right]dx
\end{equation}
