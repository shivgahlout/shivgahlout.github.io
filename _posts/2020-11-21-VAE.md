...
Using $(27)$,  
\begin{equation}
D_{KL}\left( q_{\phi}(\textbf{z}|\textbf{x})||p_{\theta}(\textbf{z})\right)=\frac{1}{2}\sum_N\left[-log({\boldsymbol{\sigma}}(\textbf{x}))+\boldsymbol{\sigma}(\textbf{x})+\boldsymbol{\mu}^2(\textbf{x})-{1}\right]
\end{equation}
i.e we are assuming prior to be $N(0,1)$. Also, note that prior
doesn't have parameters in this case.
