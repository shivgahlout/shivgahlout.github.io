...
\textbf{1. The Total Variation (TV) distance:} \\
\begin{equation}
\delta(P_0, P_{\theta})=\underset{x \in X}{sup}|P_0(x)-P_{\theta}(x)|
\end{equation}
For the above above example, 
\begin{equation}
\delta(P_0, P_{\theta})_{\{x=0\}}=|1-0|=1
\end{equation}
Similarly,
\begin{equation}
\delta(P_0, P_{\theta})_{\{x=\theta\}}=|0-1|=1
\end{equation}
However, if $\theta=0$,
\begin{equation}
\delta(P_0, P_{\theta})_{\{x=\theta=0\}}=|1-1|=0
\end{equation}
Hence, 
\begin{equation}
\begin{aligned}
...
\textbf{2. The Kullback-Leibler (KL) divergence}
\begin{equation}
...
\textbf{3. The Jensen-Shannon (JS) divergence}
...
## Training of WGANs
The pseudo-code for training the WGANs in PyTorch can be given as follows. Let generator and critic are implemented using a neural network. $z$ be noise sample given to the generator for producing the fake samples. Optim\_G and Optim\_C be the optimizer for generator and critic, respectively. n\_critic be the number of iterations for training the critic, and c be clipping parameter. 
